...
<dvi>
    <form method="POST" action="{{ route('store') }}" enctype="multipart/form-data">
        ...
        <div class="flex flex-col gap-5">
            ...
            <input type="text" name="name" value="{{ old('name') }}" class="bg-yellow-300 py-2 px-4 rounded">
                @error('name')
                    <p class="text-red-600">{{ $message }}</p>
                @enderror

            <label for="">Description</label>
            <input type="text" name="description" value="{{ old('description') }}" class="bg-red-300 py-2 px-4 rounded">
